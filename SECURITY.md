...
deployment is intended for one trusted user on one machine. The retrieval API has no built-in
authentication, authorization, rate limiting, or tenant isolation, and `/ingest` can start a
local ingestion subprocess. Do not expose it to a LAN or the public internet.
